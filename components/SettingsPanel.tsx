    ...
    <>
      ...
      <div className="fixed right-0 top-0 bottom-0 w-full md:w-96 bg-black/90 backdrop-blur-xl border-l border-white/10 z-50 p-6 overflow-y-auto animate-fade-in">
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-serif text-white">Background Settings</h2>
          <button
            onClick={onClose}
        ...
        {/* Upload Section */}
        <div className="mb-8">
          <h3 className="text-sm text-white/60 mb-3 uppercase tracking-wider">Custom Photos</h3>
          ...
          <label className="group cursor-pointer relative px-4 py-3 bg-white/5 border border-white/10 hover:bg-white/10 transition-all rounded-lg overflow-hidden backdrop-blur-sm flex items-center justify-center gap-2">
            ...
            <span className="text-sm text-white/80 font-medium">Upload Photo</span>
            <input
              type="file"
              accept="image/*"
          ...
          <p className="text-xs text-white/40 mt-2">
            Upload up to 5 photos to customize your background
          </p>
        </div>
        ...
        {uploadedPhotos.length > 0 && (
          <div className="mb-8">
            <h3 className="text-sm text-white/60 mb-3 uppercase tracking-wider">Your Photos</h3>
        ...
        <div className="p-4 bg-white/5 rounded-lg border border-white/10">
          <h3 className="text-sm text-white/60 mb-2 uppercase tracking-wider">Default Background</h3>
          <p className="text-sm text-white/80">
            {selectedCustomPhotoId
              ? 'Using your custom photo. Click it again to switch back to default.'
              : 'Using beautiful default backgrounds that rotate automatically.'}
          </p>
        </div>

        {/* Instructions */}
        <div className="mt-8 space-y-2 text-xs text-white/40">
          <p>• Click a photo to use it as your background</p>
          <p>• Click again to switch back to default</p>
          <p>• Hover and click trash icon to delete a photo</p>
        </div>
      </div>
    </>
  );
